...
DecoderStateMachine::onDecodeStreamHeader(Buffer::Instance& buffer) {
  ...
  if (!ret.second) {
    ENVOY_LOG(debug, "jres decoder: need more data for {} protocol", protocol_.name());
  ...
  if (metadata->messageType() == MessageType::HeartbeatRequest ||
    ...
    if (buffer.length() < (context->headerSize() + context->bodySize())) {
      ENVOY_LOG(debug, "jres decoder: need more data for {} protocol heartbeat", protocol_.name());
      return {ProtocolState::WaitForData};
    }

    ENVOY_LOG(debug, "jres decoder: this is the {} heartbeat message", protocol_.name());
...
DecoderStateMachine::onDecodeStreamData(Buffer::Instance& buffer) {
  ...
  if (!protocol_.decodeData(buffer, active_stream_->context_, active_stream_->metadata_)) {
    ENVOY_LOG(debug, "jres decoder: need more data for {} serialization, current size {}",
  ...
  ENVOY_LOG(debug, "jres decoder: ends the deserialization of the message");
  return {ProtocolState::Done};
}
...
ProtocolState DecoderStateMachine::run(Buffer::Instance& buffer) {
  while (state_ != ProtocolState::Done) {
    ENVOY_LOG(trace, "jres decoder: state {}, {} bytes available",
...
FilterStatus DecoderBase::onData(Buffer::Instance& data, bool& buffer_underflow) {
  ENVOY_LOG(debug, "jres decoder: {} bytes available", data.length());
  ...
  ENVOY_LOG(debug, "jres decoder: protocol {}, state {}, {} bytes available", protocol_.name(),
  ...
  case ProtocolState::WaitForData:
    ENVOY_LOG(debug, "jres decoder: wait for data");
  ...
  ENVOY_LOG(debug, "jres decoder: data length {}", data.length());
  return FilterStatus::Continue;
}
